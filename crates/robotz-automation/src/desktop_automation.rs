...
#[cfg(target_os = "linux")]
use robotz_core::proc::std_command;
use robotz_core::proc::tokio_command;
...
fn xi_helper_path() -> String {
    ...
    if let Ok(exe) = std::env::current_exe() {
        ...
        let p = dir.join("robotz-xi-helper");
        if p.exists() {
    ...
    if let Ok(out_dir) = std::env::var("OUT_DIR") {
        let p = std::path::Path::new(&out_dir).join("robotz-xi-helper");
        if p.exists() {
            return p.to_string_lossy().to_string();
        }
    }
